...
import { currentUserData } from '../auth.js?v=20260916d';
...
async function loadLiderData() {
    ...
    if (btnLogout) {
        btnLogout.addEventListener('click', async () => {
            const { logoutUser } = await import('../auth.js?v=20260916d');
            await logoutUser();
        });
    }

    // Carregar Atletas da Equipe
    const { getCollection } = await import('../services/db.js?v=20260916d');
